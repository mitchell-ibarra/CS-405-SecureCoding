...
#include "sqlite3.h"
// including regex header - Mitchell Ibarra
#include <regex>
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  char* error_message;
  // implement regex here to only allow appropriate characters to access the sql database - Mitchell Ibarra
  std::regex expression("or ('[[:alpha:]]+'|-?[[:digit:]]+)=('[[:alpha:]]+'|-?[[:digit:]]+);");
  // Here we check if the expression matches what is entered, and if it does then that means we have
  // potentially found an sql injection attack - Mitchell Ibarra
  if(std::regex_search(sql, expression))
  {
      std::cout << "Potential SQL Injection Found!" << std::endl;
      std::cout << "Going to prevent SQL injection and return false!" << std::endl;
      return false;
  }
  else
  {
