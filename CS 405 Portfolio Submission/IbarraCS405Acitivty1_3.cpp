// IbarraCS405Acitivty1_3.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
T add_numbers(T const& start, T const& increment, unsigned long int const& steps)
{
    ...
    // Using if statement to check if number of allowable steps is exceeded
    // based on how many whole steps are possible when we divide the max
    // by the increment - Mitchell Ibarra
    if((+std::numeric_limits<T>::max() / increment) < steps)
    {
        // If an overflow is detected, result will be returned
        // as the start value for the test_overflow() method to handle 
        // - Mitchell Ibarra
        result = start;
    }
    else
    {
        // If no overflow is detected, result will be the incremented
        // value as expected
        // - Mitchell Ibarra
...
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps)
{
    ...
    // Using if statement to check if number of allowable steps is exceeded
    // based on how many whole steps are possible when we divide the max
    // by the decrement - Mitchell Ibarra
    if((+std::numeric_limits<T>::max() / decrement) < steps)
    {
        // If an underflow is detected, result will be returned
        // as the start value for the test_underflow() method to handle 
        // - Mitchell Ibarra
        result = start; 
    }
    else
    {
        // If no underflow is detected, result will be the decremented
        // value as expected
        // - Mitchell Ibarra
        for (unsigned long int i = 0; i < steps; ++i)
        {
...
void test_overflow()
{
    ...
    // Here we check if the result is equal to the start value, in which case
    // we know that an overflow occurred so we notify the user - Mitchell Ibarra
    if (result == start)
    {
...
void test_underflow()
{
    ...
    // Here we check if the result is equal to the start value, in which case
    // we know that an underflow occurred so we notify the user - Mitchell Ibarra
    if (result == start)
    {
